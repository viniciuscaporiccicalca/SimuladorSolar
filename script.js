document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- URL DO PROXY ---
  // Usamos um proxy para contornar o erro de CORS do servidor da ANEEL.
  ...
  async function fetchDistributorsAndStates() {
    ...
    // Adicionamos o PROXY_URL antes do endereço da ANEEL
    const url = `${PROXY_URL}https://dadosabertos.aneel.gov.br/api/3/action/datastore_search?resource_id=${AGENTES_RESOURCE_ID}&q=Distribui%C3%A7%C3%A3o&limit=500`;
    ...
    try {
      ...
      console.log(
        "API Etapa 1: Mapeamento de distribuidoras por estado concluído."
      );
      return true;
    } catch (error) {
  ...
  async function fetchAllTariffs() {
    ...
    // Adicionamos o PROXY_URL antes do endereço da ANEEL
    const url = `${PROXY_URL}https://dadosabertos.aneel.gov.br/api/3/action/datastore_search?resource_id=${TARIFAS_RESOURCE_ID}&q="Convencional B1 Residencial"&limit=1000&sort=DatVigencia desc`;
    ...
    try {
      ...
      data.result.records.forEach((record) => {
        ...
        if (!tariffByDistributor[distributorName]) {
      ...
      console.log("API Etapa 2: Tarifas carregadas.");
      return true;
    } catch (error) {
  ...
  // --- FUNÇÕES DE LÓGICA DA UI (sem alterações) ---
  function popularEstados() {
    estadoSelect.innerHTML =
      '<option value="" disabled selected>Estado</option>';
    const estados = Object.keys(distributorsByState).sort();
    estados.forEach((uf) => {
      estadoSelect.add(new Option(uf, uf));
    });
