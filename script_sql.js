// Arquivo: script_sql.js (Versão otimizada com API SQL da ANEEL)
$(document).ready(function () {
  ...
  // Função principal para buscar e processar dados da ANEEL via SQL
  function fetchAneelData() {
    ...
    // URL final para a chamada da API
    const apiUrl = `https://dadosabertos.aneel.gov.br/api/3/action/datastore_search_sql?sql=${encodeURIComponent(
      sqlQuery
    )}`;
    ...
    return $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        console.error(
          "Falha ao buscar dados da ANEEL (SQL):",
          textStatus,
          errorThrown
        );
        alert(
